...
export default Kapsule({
        ...
        function parseData(rawData) {
          ...
          for (let i = 0, ilen = rawData.length; i < ilen; i++) {
            ...
            for (let j = 0, jlen = rawData[i].data.length; j < jlen; j++) {
              for (
                let k = 0, klen = rawData[i].data[j].data.length;
                k < klen;
                k++
              ) {
  ...
  methods: {
    ...
    zoomYLabels(state, _) {
      ...
      function y2Label(y) {
        ...
        return getIdxLine(
          state.completeStructData[state.completeStructData.length - 1],
          state.completeStructData[state.completeStructData.length - 1].lines
            .length - 1
        );
      ...
      function label2Y(label, useIdxAfterIfNotFound) {
        ...
        for (let i = 0, lenI = state.completeStructData.length; i < lenI; i++) {
          const grpCmp = state.grpCmpFunction(
            label.group,
            state.completeStructData[i].group
          );
          if (grpCmp < 0) break;
          if (grpCmp == 0 && label.group == state.completeStructData[i].group) {
            for (
              let j = 0, lenJ = state.completeStructData[i].lines.length;
              j < lenJ;
              j++
            ) {
              const cmpRes = state.labelCmpFunction(
                label.label,
                state.completeStructData[i].lines[j]
              );
              if (cmpRes < 0) {
              ...
              if (
                cmpRes == 0 &&
                label.label == state.completeStructData[i].lines[j]
              ) {
                return idx + j;
              }
            }
            return (
              idx + state.completeStructData[i].lines.length - subIdxNotFound
            );
          }
    ...
    sortChrono(state, asc) {
      ...
      function buildIdx(accessFunction) {
        ...
        for (let i = 0, len = state.completeFlatData.length; i < len; i++) {
          ...
          const itmList = state.completeFlatData.filter(
            (d) => key == accessFunction(d)
          );
          idx[key] = [
      ...
      function getCmpFunction(accessFunction, asc) {
        return (a, b) =>
          timeCmp(accessFunction(asc ? a : b), accessFunction(asc ? b : a));
      }
  ...
  init(el, state) {
    const elem = d3Select(el).attr("class", "timelines-chart");
    ...
    function buildDomStructure() {
      ...
      state.groupGradId = Gradient()
        .colorScale(
          d3ScaleLinear().domain([0, 1]).range(state.groupBkgGradient)
        )
        .angle(-90)(state.svg.node())
        .id();
      ...
      state.colorLegend = ColorLegend()(
        state.svg.append("g").attr("class", "legendG").node()
      );

      state.graph = state.svg.append("g");

      state.dateMarkerLine = state.svg
        .append("line")
      ...
      function invertOrdinal(val, cmpFunc) {
        ...
        if (scRange.length === 2 && scDomain.length !== 2) {
          ...
          scRange = d3Range(
            scRange[0],
            scRange[1],
            (scRange[1] - scRange[0]) / scDomain.length
          );
        }
      ...
      function addOverviewArea() {
        state.overviewArea = TimeOverview()
          ...
          .domainRange(state.zoomX)
          .currentSelection(state.zoomX)(state.overviewAreaElem.node());
        ...
        state.svg.on("zoomScent", function (event) {
          ...
          if (
            zoomX[0] < state.overviewArea.domainRange()[0] ||
            zoomX[1] > state.overviewArea.domainRange()[1]
          ) {
    ...
    function addTooltips() {
      ...
      state.lineTooltip = d3Tip()
        ...
        .html((event, d) => {
          const rightPush = d.hasOwnProperty("timeRange")
            ? state.xScale.range()[1] - state.xScale(d.timeRange[1])
            : 0;
      ...
      state.segmentTooltip = d3Tip()
        ...
        .html((event, d) => {
          ...
          const normVal =
            state.zColorScale.domain()[state.zColorScale.domain().length - 1] -
            state.zColorScale.domain()[0];
          const dateFormat = (state.useUtc ? d3UtcFormat : d3TimeFormat)(
            `${state.timeFormat}${state.useUtc ? " (UTC)" : ""}`
          );
          return (
            "<strong>" +
            d.labelVal +
            ...
            (normVal
              ? " (<strong>" +
                Math.round(
                  ((d.val - state.zColorScale.domain()[0]) / normVal) *
                    100 *
                    100
                ) /
                  100 +
    ...
    function addZoomSelection() {
      ...
      state.graph.on("mousedown", function (event) {
        ...
        if (
          startCoords[0] < 0 ||
          startCoords[0] > state.graphW ||
          startCoords[1] < 0 ||
          startCoords[1] > state.graphH
        )
          return;

        state.disableHover = true;

        const rect = state.graph
          .append("rect")
        ...
        d3Select(window)
          .on("mousemove.zoomRect", function (event) {
            ...
            state.svg.dispatch("zoomScent", {
              detail: {
                zoomX: [startCoords[0], newCoords[0]]
                  .sort(d3Ascending)
                  .map(state.xScale.invert),
                zoomY: [startCoords[1], newCoords[1]]
                  .sort(d3Ascending)
                  .map(
                    (d) =>
          ...
          .on(
            ...
            function (event) {
              ...
              const newDomainY = [startCoords[1], endCoords[1]]
                .sort(d3Ascending)
                .map(
                  (d) =>
                    state.yScale.domain().indexOf(state.yScale.invert(d)) +
              ...
              const changeY =
                newDomainY[0] != state.zoomY[0] ||
                newDomainY[1] != state.zoomY[1];
      ...
      state.resetBtn = state.svg
        ...
        .on("mouseout", function () {
          d3Select(this).style("opacity", 0.6);
        });
    }
    ...
    function setEvents() {
      ...
      state.svg.on("resetZoom", function () {
        ...
        if (
          prevZoomX[0] < newZoomX[0] ||
          prevZoomX[1] > newZoomX[1] ||
          prevZoomY[0] != newZoomY[0] ||
          prevZoomY[1] != newZoomX[1]
        ) {
          state.zoomX = [
  ...
  update(state) {
    ...
    function applyFilters() {
      ...
      state.flatData =
        state.minSegmentDuration > 0
          ? state.completeFlatData.filter(
              (d) => d.timeRange[1] - d.timeRange[0] >= state.minSegmentDuration
            )
          : state.completeFlatData;
      ...
      cntDwn.push(
        Math.max(
          0,
          (state.zoomY[1] == null ? state.totalNLines : state.zoomY[1] + 1) -
            cntDwn[0]
        )
      ); // Number of lines
      ...
      for (let i = 0, len = state.completeStructData.length; i < len; i++) {
        ...
        if (state.minSegmentDuration > 0) {
          ...
          if (
            !state.flatData.some(
              (d) => d.group == state.completeStructData[i].group
            )
          ) {
            continue; // No data for this group
          }

          validLines = state.completeStructData[i].lines.filter((d) =>
            state.flatData.some(
              (dd) =>
    ...
    function setupDimensions() {
      ...
      state.graphH = d3Min([
        state.nLines * state.maxLineHeight,
        state.maxHeight - state.topMargin - state.bottomMargin,
      ]);
      ...
      state.graph.attr(
        "transform",
        "translate(" + state.leftMargin + "," + state.topMargin + ")"
      );
        ...
        state.overviewArea
          ...
          .height(
            state.overviewHeight +
              state.overviewArea.margins().top +
              state.overviewArea.margins().bottom
          );
      }
    }
    ...
    function adjustXScale() {
      state.zoomX[0] =
        state.zoomX[0] || d3Min(state.flatData, (d) => d.timeRange[0]);
      state.zoomX[1] =
        state.zoomX[1] || d3Max(state.flatData, (d) => d.timeRange[1]);
    ...
    function adjustYScale() {
      ...
      for (let i = 0, len = state.structData.length; i < len; i++) {
        labels = labels.concat(
          state.structData[i].lines.map(function (d) {
      ...
      state.yScale.range([
        (state.graphH / labels.length) * 0.5,
        state.graphH * (1 - 0.5 / labels.length),
      ]);
    }
    ...
    function adjustGrpScale() {
      ...
      let cntLines = 0;
      state.grpScale.range(
        state.structData.map((d) => {
          const pos =
            ((cntLines + d.lines.length / 2) / state.nLines) * state.graphH;
    ...
    function adjustLegend() {
      state.svg
        ...
        .attr(
          "transform",
          `translate(${state.leftMargin + state.graphW * 0.05},2)`
        );

      state.colorLegend
        .width(Math.max(120, (state.graphW / 3) * (state.zQualitative ? 2 : 1)))
        .height(state.topMargin * 0.6)
      ...
      state.resetBtn
        .transition()
        .duration(state.transDuration)
        .attr("x", state.leftMargin + state.graphW * 0.99)
        .attr("y", state.topMargin * 0.8);

      TextFitToBox().bbox({
        width: state.graphW * 0.4,
        height: Math.min(13, state.topMargin * 0.8),
      })(state.resetBtn.node());
    }
    ...
    function renderAxises() {
      ...
      // X
      const nXTicks = Math.max(
        2,
        Math.min(12, Math.round(state.graphW * 0.012))
      );
      ...
      const labelDisplayRatio = Math.ceil(
        (state.nLines * state.minLabelFont) /
          Math.sqrt(2) /
          state.graphH /
          fontVerticalMargin
      );
      const tickVals = state.yScale
        .domain()
        .filter((d, i) => !(i % labelDisplayRatio));
      let fontSize = Math.min(
        12,
        (state.graphH / tickVals.length) * fontVerticalMargin * Math.sqrt(2)
      );
    ...
    function renderGroups() {
      let groups = state.graph
        .selectAll("rect.series-group")
        .data(state.structData, (d) => d.group);
      ...
      groups
        ...
        .attr("y", function (d) {
          return (
            state.grpScale(d.group) -
            (state.graphH * d.lines.length) / state.nLines / 2
          );
        });
    }
      ...
      newSegments
        .on("mouseover", function () {
          ...
          d3Select(this)
            ...
            .attr("y", function (d) {
              return (
                state.yScale(d.group + "+&+" + d.label) -
                (state.lineHeight + hoverEnlarge) / 2
              );
            })
        ...
        .on("mouseout", function () {
          d3Select(this)
            ...
            .attr("y", function (d) {
              return (
                state.yScale(d.group + "+&+" + d.label) - state.lineHeight / 2
              );
            })
